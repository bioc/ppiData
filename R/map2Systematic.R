map2Systematic <- function(allProt, tableList, sWAC){
    ...
    yeast2Sys <- as.list(org.Sc.sgd.db::org.Sc.sgdCOMMON2ORF)
    yeast2Sys <- yeast2Sys[!is.na(yeast2Sys)]
    yeastAlias <- names(unlist(as.list(org.Sc.sgd.db::org.Sc.sgdALIAS)))
    yro <- unlist(as.list(org.Sc.sgd.db::org.Sc.sgdREJECTORF))
    #notfound <- vector()
    #notfoundSGD <- vector()
    #notfound2 <- vector()
    #notfound2SGD <- vector()
