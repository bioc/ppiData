...
\begin{itemize}
-  \Robject{homodimer}: If FALSE, all homodimer relationships will be disregarded,
otherwise the homodimer relationships will be recorded in the adjacency matrix.
-  \Robject{unWeighted}: If TRUE, the entries of the adjacency matrix will be binary
(0,1) to account for the presence or absence of the bait to prey interaction
without regards for multiplicity.
-  \Robject{onlyRecip}: If TRUE, the matrix will be symmetric with only reciprocated
interactions recorded in the adjacency matrix.
-  \Robject{symMat}: If TRUE, the union of viable baits and viable preys will index
both the rows and columns of the adjacency matrix.
-  \Robject{baitAsPrey}: If TRUE, the union of the viable baits and viable preys will
index the columns while only the viable baits will index the rows.
-  \Robject{baitsOnly}: If TRUE, the matrix will be indexed in the row and column by
...
\begin{itemize}
-  \Robject{bpMat}: The bait to prey adjacency matrix.
-  \Robject{directed}: A logical. If TRUE, the function creates a directed
graph. If FALSE, the function attempts to create an undirected graph if possible.
-  \Robject{bp}: A logical. If TRUE, the adjacency matrix is that of a empirical
